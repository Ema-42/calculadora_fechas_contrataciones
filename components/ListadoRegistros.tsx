...
export default function ListadoRegistros({ registros }: ListadoRegistrosProps) {
  ...
  const [registrosPorPagina, setRegistrosPorPagina] = useState<number | "all">(5) // Estado para registros por página
  ...
  const registrosFiltrados = useMemo(() => {
    ...
    return registros.filter(
      (registro) =>
        ...
        registro.modalidad.toLowerCase().includes(busqueda.toLowerCase()),
    )
  }, [registros, busqueda])

  const totalPaginas = registrosPorPagina === "all" ? 1 : Math.ceil(registrosFiltrados.length / registrosPorPagina)
  const indiceInicio = (paginaActual - 1) * (typeof registrosPorPagina === "number" ? registrosPorPagina : 0)
  const registrosPaginados =
    registrosPorPagina === "all"
      ? registrosFiltrados
      : registrosFiltrados.slice(indiceInicio, indiceInicio + registrosPorPagina)
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
        ...
        <h2 className="text-xl font-semibold text-gray-800 mb-4 sm:mb-0">Registros ({registrosFiltrados.length})</h2>
      ...
      {registrosFiltrados.length === 0 ? (
        <div className="text-center py-12">
          <p className="text-gray-500 text-lg">
            {busqueda ? "No se encontraron registros que coincidan con la búsqueda" : "No hay registros aún"}
          </p>
        </div>
      ) : (
        <>
      ...
      {mostrarModalPDF && <ModalImprimirTodos registros={registros} onClose={() => setMostrarModalPDF(false)} />}
    </div>
  )
}
