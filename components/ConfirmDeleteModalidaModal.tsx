...
interface ConfirmDeleteModalidadModalProps {
  open: boolean;
...
export default function ConfirmDeleteModalidadModal({
  open,
  onClose,
  onConfirm,
  modalidadNombre,
}: ConfirmDeleteModalidadModalProps) {
  if (!open) return null;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white dark:bg-[hsl(217,26%,14%)] rounded-lg shadow-lg dark:shadow-gray-900/50 p-6 w-11/12 max-w-sm z-10">
        <h2 className="text-lg font-semibold text-gray-800 dark:text-gray-100 mb-4">
          🗑️ ¿Eliminar esta modalidad?
        </h2>
        <p className="text-sm text-gray-600 dark:text-gray-300 mb-6">
          Esta acción marcará la modalidad{" "}
          <span className="font-semibold">{modalidadNombre}</span> como eliminada
