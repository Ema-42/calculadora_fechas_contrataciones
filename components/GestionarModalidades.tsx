...
export default function GestionarModalidades({
  modalidades,
  onAgregar,
  onEliminar,
  onEditar,
}: GestionarModalidadesProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!nuevoNombre) {
      alert("Por favor complete el nombre de la modalidad");
      return;
    }
    ...
    if (
      ...
      notifyError("Ya existe una modalidad con ese nombre");
      return;
    }
  ...
  const formatearNombre = (nombre: string | null) => nombre || "-";
  ...
  const startEdit = (id: number, nombreActual?: string | null) => {
    setEditId(id);
  ...
  return (
    <>
                              ...
                              {editId === modalidad.id ? (
                                ...
                                <>
                                  <p className="font-medium text-gray-900 dark:text-gray-100 truncate">
                                    {formatearNombre(modalidad.nombre)}
                                  </p>
                                </>
                              )}
                            </div>
                            ...
                            <div className="flex items-center gap-2 ml-2 flex-shrink-0">
                              {editId === modalidad.id ? (
                                ...
                                <>
                                  <button
                                    onClick={() =>
                                      startEdit(modalidad.id, modalidad.nombre)
                                    }
                                    title="Editar"
      ...
      <ConfirmDeleteModalidadModal
        open={modalOpen}
        onClose={() => setModalOpen(false)}
        onConfirm={handleConfirmDelete}
        modalidadNombre={itemNombreSeleccionado}
      />
    </>
  );
}
