...
export default function TablaRegistros({ registros }: TablaRegistrosProps) {
  ...
  const handleDownloadSingle = (registro: Registro, event: React.MouseEvent) => {
  ...
  return (
    <div className="grid grid-cols-1 gap-4">
      {registros.length === 0 ? (
        <div className="col-span-1 text-center py-8 text-gray-500">No hay registros para mostrar.</div>
      ) : (
        registros.map((registro) => (
          <div
            key={registro.id}
            className={`bg-gray-50 rounded-lg shadow-md cursor-pointer transition-all duration-200 ease-in-out 
              ${filaSeleccionada === registro.id ? "bg-gray-100 border-l-4 border-gray-500" : "hover:bg-gray-100"}`}
            onClick={() => setFilaSeleccionada(filaSeleccionada === registro.id ? null : registro.id)}
          >
            {/* Sección de Encabezado de la Tarjeta */}
            <div className="bg-red-100 p-4 rounded-t-lg mb-3">
              <div className="flex justify-between items-center mb-2">
                <span className="text-sm font-semibold text-gray-700">ID: #{registro.id}</span>
                <div className="flex items-center text-sm text-gray-700">
                  <CalendarClock className="mr-1 text-gray-500" size={14} />
                  <span>{formatearFecha(registro.fechaGeneracion, true)}</span>
                </div>
              </div>

              <h3 className="text-lg font-bold text-red-900 uppercase text-center mb-2">
                {registro.titulo || "Sin Título"}
              </h3>
              <div className="w-full text-center">
                <span className="inline-block px-3 py-1 text-sm font-semibold rounded-full bg-red-200 text-red-800 w-full">
                  {registro.modalidad}
                </span>
              </div>
            </div>
            ...
            <div className="p-4 pt-0">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm text-gray-700 mb-3">
                <div className="flex items-center">
                  ...
                  <p>
                    <strong className="text-gray-800">F. Inicio:</strong> {formatearFecha(registro.fechaInicio)}
                  </p>
                </div>
                <div className="flex items-center">
              ...
              <div className="border-t border-gray-200 pt-3 mt-3">
                ...
                <ul className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-1 text-sm text-gray-700">
                  <li className="flex items-center">
                    ...
                    <strong className="text-gray-800">Publicación:</strong> {formatearFecha(registro.fechaPublicacion)}
                  </li>
                  <li className="flex items-center">
                    <CalendarCheck className="mr-2 text-gray-500" size={16} />
                    <strong className="text-gray-800">Apertura:</strong> {formatearFecha(registro.fechaApertura)}
                  </li>
                  <li className="flex items-center">
                    <CalendarCheck className="mr-2 text-gray-500" size={16} />
                    <strong className="text-gray-800">Adjudicación:</strong>{" "}
                    {formatearFecha(registro.fechaAdjudicacion)}
                  </li>
                  <li className="flex items-center">
                    <CalendarCheck className="mr-2 text-gray-500" size={16} />
                    <strong className="text-gray-800">Presentación Docs:</strong>{" "}
                    {formatearFecha(registro.fechaPresentacionDocs)}
                  </li>
                  <li className="flex items-center">
                    <CalendarCheck className="mr-2 text-gray-500" size={16} />
                    <strong className="text-gray-800">Firma Contratos:</strong>{" "}
